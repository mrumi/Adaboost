...
public class ensemble {
    ...
    public void testSingleData(Node node, Data row)
    {
        ...
        {
            if(row.attributes[node.getAttr()]==node.children[i].getVal())
    ...
    public void createTree(Node node)
    {
        ...
        {
        	node.setLeaf(setResult_ID3(node));        	
       		return;
        }
    ...
    public void decisionStump()
    {
        boolean selected=false;
        double bestGain=0;
        root.setEntropy(calEntropy(root));
        int selected_attr=-1;
        for(int i=0;i<count_attr-1;i++)
        {
            double avgEntropy=0;
            ...
            double gain=root.getEntropy()-avgEntropy;
            if (selected==false)
            {
                selected=true;
                bestGain=gain;
